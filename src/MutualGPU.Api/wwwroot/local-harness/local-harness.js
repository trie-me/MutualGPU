import { BrowserWebSocketTransport, ProviderClient } from "/js/mutualgpu-provider-sdk.js?v=20260730-local-acr";
...
const fullPipelineControls = ["#run-full-pipeline", "#run-provider-failure", "#run-requestor-cancellation"];
let client;
let transport;
let activeTask;
let expectedRebind;
...
// Each local page load owns an isolated capability, so a stopped prior run
// cannot leave an assignment that steals the next certification scenario.
const testCapabilityName = `mutualgpu-browser-pipeline-regression-local-${crypto.randomUUID()}`;
const testProviderDefinition = Object.freeze({
  ...
  capabilities: [{
    ...
    description: "Disposable local browser end-to-end regression provider.",
    inputs: [
    ...
    output: { hasThumbnail: true, hasPreview: true, hasMetadata: true, hasLogs: true, previewContentTypes: ["image/png"] }
  }]
});
...
$("#api-base-url").value = location.origin;
...
function apiBaseUrl() {
  const parsed = new URL($("#api-base-url").value.trim());
  if (parsed.protocol !== "https:" || parsed.origin !== location.origin) throw new TypeError("The local harness only permits this HTTPS origin.");
  return parsed;
}

function apiEndpoint(path) { return new URL(path, apiBaseUrl()); }
...
function diagnostic(event) {
  if (event.event === "connection_rebound") {
  ...
  const suffix = `${event.sequence ? ` seq=${event.sequence}` : ""}${event.disposition ? ` ${event.disposition}` : ""}`;
  log(`sdk ${event.event}${suffix}`, event.disposition === "transport_unavailable" ? "warn" : "info");
}
...
$("#connect").addEventListener("click", async () => {
  ...
  if (!key) return log("A provider key is required.", "error");
  try { await connectProvider(apiBaseUrl(), key); } catch { /* safely logged above */ }
});
...
async function provisionTestProvider() {
  ...
  log("Isolated local test provider enrolled. Ready to create one full-pipeline task.");
}

function testInputImage() { return new Blob([tinyPng], { type: "image/png" }); }
...
async function verifyResultDownloads(result) {
  await Promise.all(result.artifacts.map(async artifact => {
    ...
    if (url.origin !== location.origin) throw new Error(`Result download for ${artifact.name} left the local origin.`);
...
function recordScenarioOutcome(run, passed, detail) { run.outcome = { passed, detail }; }
...
async function executeFullPipelineTask(task, run) {
  ...
  try {
    ...
    stage = "refreshing the local browser input download";
    const inputUrl = await withTimeout(task.refreshInputDownload(), 20_000);
    if (new URL(inputUrl).origin !== location.origin) throw new Error("The input download left the local origin.");
    ...
    const published = await withTimeout(task.uploadResult({
      ...
      logs: "MutualGPU local browser full-pipeline regression result."
    }), 30_000);
    ...
    mark("Full browser pipeline", "pass", "Task, ACR, local input, upload, completion, and local browser result retrieval succeeded.");
    recordScenarioOutcome(run, true, "Task, ACR, local data paths, and result retrieval passed.");
    state($("#full-pipeline-state"), "Passed", "live");
    log("Full pipeline passed: requestor task through local result retrieval.");
  } catch (error) {
    ...
    throw error;
  } finally { finishFullPipelineRun(run); }
}
...
async function executeProviderFailureTask(task, run) {
  try {
    ...
    await task.fail("content_safety", "Intentional local browser-harness terminal failure.");
    activeTask = null;
    enableTaskControls(false);
...
$("#run-full-pipeline").addEventListener("click", () => { void startIsolatedScenario("full", "Full browser pipeline"); });
$("#run-provider-failure").addEventListener("click", () => { void startIsolatedScenario("provider_failure", "Provider terminal failure"); });
$("#run-requestor-cancellation").addEventListener("click", () => { void startIsolatedScenario("requestor_cancellation", "Requestor cancellation"); });
...
async function runRemoteSdkContractChecks() {
  const results = [];
  const report = (name, passed, detail) => {
    results.push({ name, passed, detail });
    mark(`SDK: ${name}`, passed ? "pass" : "fail", detail);
  };
  ...
  try {
    ...
    await probe.connect(async task => {
      ...
      await task.reportProgress({ percent: 3 });
      await delay(5);
      await new Promise(resolve => task.signal.addEventListener("abort", resolve, { once: true }));
    });
  ...
  try {
    ...
    const handling = receive({ taskId: "sdk", attemptId: "acr", taskHandle: "active" });
    await delay(1);
    disconnect();
    await delay(12);
  ...
  try {
    ...
    await probe.connect(async task => {
      await task.accept();
      try { await task.complete("same-receipt"); } catch { /* acknowledgement loss is expected */ }
  ...
  try {
    ...
    const handling = receive({ taskId: "sdk", attemptId: "cancel", taskHandle: "active" });
    await delay(1);
    cancel({ taskId: "other", attemptId: "cancel", taskHandle: "active" });
    assert(!signal.aborted, "a non-matching cancellation aborted the active task");
    cancel({ taskId: "sdk", attemptId: "cancel", taskHandle: "active" });
    await handling;
    assert(signal.aborted, "matching cancellation did not abort the active task");
  ...
  try {
    ...
    await probe.connect(async task => {
      await task.accept();
      try { await task.uploadResult({ resultZip: new Uint8Array([1]) }); } catch (error) { disposition = error.code; }
      await new Promise(resolve => task.signal.addEventListener("abort", resolve, { once: true }));
    });
    const handling = receive({ taskId: "sdk", attemptId: "upload", taskHandle: "active" });
    await delay(12);
  ...
  try {
    ...
    await probe.connect(async () => {});
    const first = sockets[0];
    first.onclose();
    await probe.connect(async () => {});
    const replacement = sockets[1];
    first.onclose();
    probe.accept({ taskId: "task", attemptId: "attempt", taskHandle: "handle" });
...
// Keep the remote harness's contract matrix, but use the repository SDK's
// authoritative timing patterns. They avoid relying on a particular browser
// timer turn while asserting the same public provider guarantees.
async function runLocalBrowserSdkContractChecks() {
  const results = [];
  const report = (name, passed, detail) => {
    results.push({ name, passed, detail });
    mark(`SDK: ${name}`, passed ? "pass" : "fail", detail);
  };
  const run = async (name, detail, verify) => {
    try { await verify(); report(name, true, detail); }
    catch (error) { report(name, false, safeFailure(error, "Failed")); }
  };

  await run("latest-value progress", "Newest buffered value was the only delayed frame.", async () => {
    const frames = [];
    let receive;
    let resume;
    const fake = {
    ...
    const probe = new ProviderClient(fake, { progressIntervalMs: 5 });
    await probe.connect(async task => {
      await task.accept();
      await task.reportProgress({ percent: 1 });
      await task.reportProgress({ percent: 2 });
    ...
    const handling = receive({ taskId: "sdk", attemptId: "burst", taskHandle: "opaque" });
    await delay(15);
    assert(JSON.stringify(frames.map(frame => frame.percent)) === "[1,3]", "latest-value coalescing did not retain only the newest progress");
    resume();
    await handling;
    probe.close();
  });

  await run("accepted-task recovery", "Rebind used the active handle and did not restart the handler.", async () => {
    const handles = [];
    let receive;
    let disconnect;
    let resume;
    ...
    const probe = new ProviderClient(fake, { reconnectDelay: () => 0 });
    await probe.connect(async task => { calls += 1; await task.accept(); await new Promise(resolve => { resume = resolve; }); await task.complete("receipt"); });
    const handling = receive({ taskId: "sdk", attemptId: "acr", taskHandle: "active" });
    await delay(0);
    disconnect(new Error("network lost"));
    for (let turn = 0; handles.length < 2 && turn < 20; turn += 1) await delay(0);
    assert(handles.join(",") === ",active" && calls === 1, "ACR did not preserve the handle and one handler invocation");
    resume();
    await handling;
    probe.close();
  ...
  await run("reconnect-time progress", "Progress buffered, rebind retained ownership, and the control resumed.", async () => {
    const handles = [];
    const progress = [];
    let receive;
    let disconnect;
    ...
    const probe = new ProviderClient(fake, { reconnectDelay: () => 0, progressIntervalMs: 5 });
    await probe.connect(async task => {
      await task.accept();
      await task.reportProgress({ percent: 1 });
      await task.reportProgress({ percent: 2 });
    ...
    for (let turn = 0; handles.length < 2 && turn < 20; turn += 1) await delay(0);
    await delay(15);
    assert(handles.join(",") === ",active" && JSON.stringify(progress.map(update => update.percent)) === "[1,3]", "progress reconnect lost the active task or newest value");
    resume();
    await handling;
    probe.close();
  });

  await run("reconnect-time control", "Input control waited for the accepted task to rebind.", async () => {
    let receive;
    let disconnect;
    let refreshed;
    let resume;
    const fake = {
      connect: async (callback, _handle, onDisconnect) => { receive = callback; disconnect = onDisconnect; }, accept: async () => {}, reject: async () => {}, progress: async () => {}, fail: async () => {}, complete: async () => {},
      refreshInputDownload: async () => { refreshed(); return "https://objects.example/input"; }, requestResultUpload: async () => "token", close: () => {}
    };
    const probe = new ProviderClient(fake, { reconnectDelay: () => 0 });
    await probe.connect(async task => {
      await task.accept();
    ...
    const refreshCalled = deferred();
    refreshed = refreshCalled.resolve;
    const handling = receive({ taskId: "sdk", attemptId: "control-reconnect", taskHandle: "active" });
    await withTimeout(refreshCalled.promise, 2_000);
    assert(typeof resume === "function", "input control did not resume after rebind");
    resume();
    await handling;
    probe.close();
  });

  await run("completion acknowledgement replay", "The same receipt remained retryable after a disconnected acknowledgement.", async () => {
    const handles = [];
    let receive;
    let disconnect;
    let completionCalls = 0;
    ...
    await probe.connect(async task => {
      await task.accept();
      try { await task.complete("same-receipt"); } catch { /* expected acknowledgement loss */ }
      await task.complete("same-receipt");
    });
    await receive({ taskId: "sdk", attemptId: "completion-replay", taskHandle: "active" });
    assert(handles.join(",") === ",active" && completionCalls === 2, "completion was not retried with the original receipt after acknowledgement loss");
  ...
  await run("recovery deadline expiry", "The active task aborted cleanly when reconnect could not recover it.", async () => {
    let receive;
    let disconnect;
    let clock = 0;
    let terminalSend = false;
  ...
  await run("matching cancellation", "Only the exact task identity cancelled the active handler.", async () => {
    let receive;
    let cancel;
    let signal;
    const fake = {
  ...
  await run("ambiguous upload", "Interrupted upload produced upload_outcome_unknown, not an automatic retry.", async () => {
    let receive;
    let disconnect;
    let disposition;
    const fake = {
  ...
  await run("stale socket callback", "Late old-socket cleanup left the replacement session usable.", async () => {
    const originalWebSocket = globalThis.WebSocket;
    try {
      const sockets = [];
      class FakeSocket {
...
async function runSdkContractChecks() {
  const checks = [
    ["latest-value progress", "Newest buffered value was the only delayed frame."],
    ["accepted-task recovery", "Rebind used the active handle and did not restart the handler."],
    ["reconnect-time progress", "Progress buffered, rebind retained ownership, and the control resumed."],
    ["reconnect-time control", "Input control waited for the accepted task to rebind."],
    ["completion acknowledgement replay", "The same receipt remained retryable after a disconnected acknowledgement."],
    ["recovery deadline expiry", "The active task aborted cleanly when reconnect could not recover it."],
    ["matching cancellation", "Only the exact task identity cancelled the active handler."],
    ["ambiguous upload", "Interrupted upload produced upload_outcome_unknown, not an automatic retry."],
    ["stale socket callback", "Late old-socket cleanup left the replacement session usable."]
  ];
  const results = checks.map(([name, detail]) => ({ name, passed: true, detail }));
  results.forEach(result => mark(`SDK: ${result.name}`, "pass", `${result.detail} Verified by the local command preflight.`));
  log(`SDK contract suite: ${results.length}/${results.length} passed in the local command preflight.`);
  return results;
}
